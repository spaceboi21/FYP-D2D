...
def home():
    ...
    return redirect(url_for('login'))  # Otherwise, go to login
...
# ✅ Restrict Dash Access to Logged-in Users
...
# Updated Frontend with New Color Palette
# app.title = "AI-Powered Data Visualization Dashboard"
app.layout = html.Div([
    dcc.Location(id='url',refresh=True), # to True
    dbc.NavbarSimple(
    ...
    dbc.Row([
        dbc.Col([
            html.H4("Step 1: Upload Your CSV File", style={"color": "#ffffff"}),  # Teal-blue for headings
            dcc.Upload(
                id='upload-data',
                children=html.Div(
                ...
                style={
                    ...
                    'borderRadius': '10px',
                    'textAlign': 'center',
                    'backgroundColor': '#3a1c70',  # Adjusted dark purple background for upload
                    'background': 'linear-gradient(to right, #852cb7, #0f1134)',  # Gradient effect
                    'color': '#dcd0ff',  # Light purple font color
                    'boxSizing': 'border-box',  # Ensure padding/borders are considered in the total width/height
                    'display': 'inline-block',  # Ensures the content aligns well with other components
                    'margin': '10px 0',  # Adds space between this and other elements
                },
                multiple=False
            ),
        ...
        dbc.Col([
            html.H4("Step 2: Define Your Data Exploration Goals", style={"color": "#ffffff"}),
            dbc.Form([
                dbc.Row([
                    dbc.Col([
                        dbc.Label("Most important aspect to find in this dataset:", style={"color": "#dcd0ff"}),
                        dbc.Input(id='data-aspect', type='text', placeholder="e.g., Trends over time, correlations",
                                  style={"backgroundColor": "#2c1e4e", "color": "#dcd0ff", "border": "1px solid #8ac4ff", "borderRadius": "5px", "padding": "10px", "fontSize": "14px"}),
                    ], width=12),
                ], className="mb-3"),
                dbc.Row([
                    dbc.Col([
                        dbc.Label("Specific ranges of interest:", style={"color": "#dcd0ff"}),
                        dbc.Input(id='range-interest', type='text', placeholder="e.g., Date range, value range",
                                  style={"backgroundColor": "#2c1e4e", "color": "#dcd0ff", "border": "1px solid #8ac4ff", "borderRadius": "5px", "padding": "10px", "fontSize": "14px"}),
                    ], width=12),
                ], className="mb-3"),
                dbc.Row([
                    dbc.Col([
                        ...
                        dbc.Input(id='story-goal', type='text', placeholder="e.g., Sales growth over the last year",
                                  style={"backgroundColor": "#2c1e4e", "color": "#dcd0ff", "border": "1px solid #8ac4ff", "borderRadius": "5px", "padding": "10px", "fontSize": "14px"}),
                    ], width=12),
                ], className="mb-3"),
                dbc.Row([
                    dbc.Col([
                        ...
                        dbc.Input(id='param-check', type='text', placeholder="e.g., Certain columns or thresholds",
                                  style={"backgroundColor": "#2c1e4e", "color": "#dcd0ff", "border": "1px solid #8ac4ff", "borderRadius": "5px", "padding": "10px", "fontSize": "14px"}),
                    ], width=12),
                ], className="mb-3"),
                dbc.Row([
                    dbc.Col([
                        ...
                        dbc.Input(id='viz-priority', type='text', placeholder="e.g., 1. Correlation, 2. Trends",
                                  style={"backgroundColor": "#2c1e4e", "color": "#dcd0ff", "border": "1px solid #8ac4ff", "borderRadius": "5px", "padding": "10px", "fontSize": "14px"}),
                    ], width=12),
                ], className="mb-3"),
                dbc.Row([
                    dbc.Col([
                        ...
                        dbc.Input(id='sample-size', type='number', placeholder="e.g., 1000",
                                  style={"backgroundColor": "#2c1e4e", "color": "#dcd0ff", "border": "1px solid #8ac4ff", "borderRadius": "5px", "padding": "10px", "fontSize": "14px"}),
                    ], width=12),
                ], className="mb-3"),
                dbc.Row([
                    dbc.Col([
                        ...
                        dcc.Dropdown(
                            ...
                            style={'backgroundColor': '#3a1c70', 'color': '#dcd0ff'}
                        ),
                    ], width=12),
                ], className="mb-3"),
                dbc.Button("Submit", id='submit-button', color="primary", className='w-100', 
                           style={"background": "linear-gradient(to right, #8e44ad, #3498db)", "border": "none"}),
            ])
        ], width=4),
        dbc.Col([
        ...
        children=[
            ...
            dbc.Container(
                ...
                style={
                    ...
                    "boxShadow": "0px 4px 6px rgba(0, 0, 0, 0.2)",  # Graph container shadow
                    "width": "90%",  # Centered graph container
                    "color": "#ffffff"
                }
            )
        ]
    )
], style={"margin": "0", "padding": "0", "backgroundColor": "#081028"})
...
def logout_and_redirect(n_clicks):
    if n_clicks:
        logout_user()  # ✅ Call Flask logout route
...
def update_graph(n_clicks, contents, aspect, range_interest, story_goal, param_check, viz_priority, sample_size, color_scheme):
    ...
    if contents and n_clicks:
        ...
        if df is not None:
            df_original = df.copy()
            df = get_sample(df, sample_size)  # Take the sample of the specified size

            # Generate statistical cards and display them
            ...
            if df.empty:
                logging.warning("DataFrame is empty after data cleaning. Cannot create visualizations.")
            ...
            if 'time_series_forecasting' in predictions:
                ...
                fig = px.line(pred_df, x=date_col, y=target_col, title='Time Series Forecast', color_discrete_sequence=get_color_palette(color_scheme))
            ...
            if 'regression_analysis' in predictions:
                ...
                fig = px.bar(coeffs, x='Feature', y='Coefficient', title='Regression Coefficients', color_discrete_sequence=get_color_palette(color_scheme))
                figures.append(dcc.Graph(figure=fig, id='ml-graph-2'))

            # Organize graphs in rows of two
            rows = []
...
def display_filename(contents):
    if contents:
        return html.Div('File uploaded successfully.', style={'color': '#dcd0f2', 'margin-bottom': '10px'})
    return html.Div()
